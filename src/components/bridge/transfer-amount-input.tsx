    ...
    <div className={cn("flex items-center gap-2", className)}>
      ...
      <div className="flex-1 relative">
        <input type="text" inputMode="decimal" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeHolder}  disabled={disabled}
               className={cn("w-full px-4 py-3 pr-16", "bg-white border border-slate-300", "text-xl font-semibold text-slate-900 rounded-md", "focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500", "tabular-nums", "disabled:bg-slate-50 disabled:text-slate-400")} />
        {/*Unit display*/}
